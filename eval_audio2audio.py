"""
方言音声 → 標準語「音声」変換 評価スクリプト (lora_audio2audio_v1 用)

eval_convert.py の predict() 構造 + test_interleaved.py の音声デコード/保存パターンを統合。
generate_interleaved の出力からテキスト(CER評価用)と音声(WAV保存)の両方を取り出す。

Usage:
    uv run python eval_audio2audio.py --lora_path checkpoints/lora_audio2audio_v1/best --n 6
"""
...
def predict(model, processor, audio: np.ndarray, max_new_tokens: int = MAX_NEW_TOKENS):
    """方言音声 → (標準語テキスト, 音声波形 or None, 推論時間)"""
    ...
    waveform = None
    if audio_frames:
        codes = torch.stack(audio_frames, dim=1).unsqueeze(0).to(DEVICE)  # (1, 8, T)
...
def main():
    ...
    parser.add_argument("--lora_path", default="checkpoints/lora_audio2audio_v1/best")
    parser.add_argument("--n", type=int, default=6, help="評価サンプル数")
    parser.add_argument("--max_tokens", type=int, default=MAX_NEW_TOKENS)
    parser.add_argument("--output_dir", default="eval_audio2audio_results")
    args = parser.parse_args()

    model, processor = load_model_with_lora(args.lora_path)
    pairs = load_pairs()

    # data/prepare_conversion_audio2audio.py と同じ split ロジック (osaka only, seed=42, test=先頭10%)
    print("Loading test split (osaka, same seed=42 split as training data)...")
    ...
    out_jsonl = out_dir / "results_audio2audio.jsonl"
